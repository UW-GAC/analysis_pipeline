...
if (is_binary) {
    ggplot(dat, aes_string(outcome)) + geom_bar() + stat_count(geom="text", aes_string(label="..count..", vjust=-0.5))
} else {
    ggplot(dat, aes_string(outcome)) + geom_histogram()
}
```
...
genotype to test for association. The density curve for each group is shown; the
heavy black line is the density curve pooled across all groups.
