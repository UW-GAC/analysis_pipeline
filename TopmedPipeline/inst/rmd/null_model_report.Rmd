...
if (!is.null(group_var)) {
  ...
  text <- paste(
    ...
    values (`resid.cholesky`) by heterogeneous residual variance group. The `resid.cholesky` values
    are adjusted for fixed effect covariates, random effects, and heterogeneous
    residual variances. The variation of `resid.cholesky` within each group should be
    similar."
  )
  cat(text)
}
...
The adjusted phenotype values (`resid.cholesky`) are adjusted for fixed effect covariates,
...
The projected phenotype values (`resid.PY`) are the phenotype values after adjusting
...
if (is_binary) {
  ...
  if (!is.null(group_var)) {
    ...
    extra_text <- "A histogram of `resid.PY` values is shown below."
  }
  if(!is_invnorm) {
    extra_text <- paste(
      extra_text,
      "\n\nIt may be problematic for association testing if this distribution is far
      from Gaussian. This problem can be alleviated by rank normalizing `resid.PY` and
