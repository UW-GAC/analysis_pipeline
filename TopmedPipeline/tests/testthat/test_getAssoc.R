...
test_that("single related", {
    ...
    a1 <- assocTestSingle(seqData, nullmod, BPPARAM=BPPARAM, verbose=FALSE)
    seqSetFilterChrom(seqData, include=2, verbose=FALSE)
    a2 <- assocTestSingle(seqData, nullmod, BPPARAM=BPPARAM, verbose=FALSE)
...
test_that("window, burden", {
    ...
    a1 <- assocTestAggregate(seqData, nullmod, BPPARAM=BPPARAM, verbose=FALSE)
    seqSetFilterChrom(seqData, include=2, verbose=FALSE)
    seqData <- SeqVarWindowIterator(seqData, verbose=FALSE)
    a2 <- assocTestAggregate(seqData, nullmod, BPPARAM=BPPARAM, verbose=FALSE)
    files <- c(tempfile(), tempfile())
    save(a1, file=files[1])
    save(a2, file=files[2])
...
test_that("aggregate, skat", {
    ...
    a1 <- assocTestAggregate(seqData, nullmod, BPPARAM=BPPARAM, test="SKAT", verbose=FALSE)
    seqResetFilter(seqData, verbose=FALSE)
    agg <- GRangesList(gr[301:400], gr[401:500], gr[501:600])
    names(agg) <- letters[4:6]
    seqData <- SeqVarListIterator(seqData, variantRanges=agg, verbose=FALSE)
    a2 <- assocTestAggregate(seqData, nullmod, BPPARAM=BPPARAM, test="SKAT", verbose=FALSE)
    files <- c(tempfile(), tempfile())
    save(a1, file=files[1])
    save(a2, file=files[2])
    a <- rbind(a1$results, a2$results) %>%
...
test_that("aggregate, multiple chromosomes in one aggregation unit", {
  ...
  names(agg) <- "a"
  seqData <- SeqVarListIterator(seqData, variantRanges=agg, verbose=FALSE)
  a <- assocTestAggregate(seqData, nullmod, BPPARAM=BPPARAM, test="SKAT", verbose=FALSE)
  file <- tempfile()
  save(a, file = file)
...
test_that("single, binomirare", {
  ...
  seqSetFilterChrom(seqData, include=1, verbose=FALSE)
  a1 <- assocTestSingle(seqData, nullmod, BPPARAM=BPPARAM, verbose=FALSE, test="BinomiRare")
  seqSetFilterChrom(seqData, include=2, verbose=FALSE)
  a2 <- assocTestSingle(seqData, nullmod, BPPARAM=BPPARAM, verbose=FALSE, test="BinomiRare")
  files <- c(tempfile(), tempfile())
  save(a1, file=files[1])
  save(a2, file=files[2])
  a <- rbind(a1, a2)
...
test_that("window, smmat", {
    ...
    seqSetFilterChrom(seqData, include=1, verbose=FALSE)
    seqData <- SeqVarWindowIterator(seqData, verbose=FALSE)
    a1 <- assocTestAggregate(seqData, nullmod, BPPARAM=BPPARAM, test="SMMAT", verbose=FALSE)
    seqSetFilterChrom(seqData, include=2, verbose=FALSE)
    seqData <- SeqVarWindowIterator(seqData, verbose=FALSE)
    a2 <- assocTestAggregate(seqData, nullmod, BPPARAM=BPPARAM, test="SMMAT", verbose=FALSE)
    files <- c(tempfile(), tempfile())
    save(a1, file=files[1])
    save(a2, file=files[2])
    a <- rbind(a1$results, a2$results) %>%
...
test_that("window, skato", {
    ...
    seqSetFilterChrom(seqData, include=1, verbose=FALSE)
    seqData <- SeqVarWindowIterator(seqData, verbose=FALSE)
    a1 <- assocTestAggregate(seqData, nullmod, BPPARAM=BPPARAM, test="SKATO", rho=c(0,1), verbose=FALSE)
    seqSetFilterChrom(seqData, include=2, verbose=FALSE)
    seqData <- SeqVarWindowIterator(seqData, verbose=FALSE)
    a2 <- assocTestAggregate(seqData, nullmod, BPPARAM=BPPARAM, test="SKATO", rho=c(0,1), verbose=FALSE)
    files <- c(tempfile(), tempfile())
    save(a1, file=files[1])
    save(a2, file=files[2])
    a <- rbind(a1$results, a2$results) %>%
...
test_that("combine single", {
    ...
    a <- assocTestSingle(seqData, nullmod, BPPARAM=BPPARAM, verbose=FALSE)
...
test_that("combine window", {
    ...
    seqSetFilterChrom(seqData, include=1:2, verbose=FALSE)
    seqData <- SeqVarWindowIterator(seqData, verbose=FALSE)
    a <- assocTestAggregate(seqData, nullmod, BPPARAM=BPPARAM, verbose=FALSE)
